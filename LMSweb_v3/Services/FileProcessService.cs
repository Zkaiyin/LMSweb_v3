...
public class FileProcessService
{
    ...
    /// <summary>
    /// 處理上傳的學生資料檔案，檢查是否有重複的學生資料、學號是否已經存在SQL、學生性別是否為男或女
    /// </summary>
    ...
    public async Task<ProcessingResultViewModel?> ProcessingStudentData(IFormFile uploadedFile)
    {
        ...
        // 檢查學生資料是否有重複
        var studentIds = data.Select(x => x.StudentId).ToList();
        foreach (var sid in studentIds)
        {
            if (studentIds.Where(x => x == sid).Count() > 1)
            {
                result.Message = "學生資料有重複";
                return result;
            }
        }
        ...
        {
            if (_context.Query<Student>().Any(x=>x.StudentId == item))
            {
                result.Message = "學號已經存在";
                return result;
            }
        }

        // 檢查學生性別是否為男或女
        if (data.Where(x => x.StudentSex != "男" && x.StudentSex != "女").Count() > 1)
        {
